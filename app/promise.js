/*
	...
	状态转变只能是 pending -> resoleved 或者 pending -> rejected。状态转变不可逆。
	then 方法可以被同一个 promise 调用多次(每次返回的时候, new一个新的promise对象，可以进行链式调用)。
*/
/*
	Promise是一个构造函数，自己身上有all、reject、resolve这几个方法，原型上有then、catch等同样很方法
...
function Promise(executor) {
	...
	//resolve作用是保存promise的值，改变状态，如果有回调再执行回调。(不是用来执行主函数)
